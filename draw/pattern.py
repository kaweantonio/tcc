...
def draw_piece_R(dimensions, id_, x,y):
  ...
  tex.write(
    ...
    '('+str(x)+','+str(y)+') rectangle ('+str(x1)+','+str(y1)+') node [pos=.5] {'+str(id_)+'};\n'
  )
...
def draw_piece_C(pieceC, x,y):
  ...
  if location == general.HORIZONTAL:
    ...
    '++(0,'+str(y2)+') -- ' \
    '++('+str(-x1)+',0) -- ' \
    '++(0,'+str(-y1)+') node [right, pos=0.5] {'+str(pieceC.combination.piece1_id)+'};\n'
    )
    ...
    tex.write(
      ...
      '++(0,'+str(y2-y1)+') -- ' \
      '++('+str(x2-x1)+',0) -- ' \
      '++(0,'+str(-y2)+') node [right, pos=0.5] {'+str(pieceC.combination.piece2_id)+'};\n'
    )
    x = aux_x
  else:
    aux_x = x
